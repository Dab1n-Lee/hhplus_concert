...
본 과제는 Redis를 활용하여 대용량 트래픽을 효율적으로 처리하기 위한 두 가지 기능을 구현하는 것.
...
콘서트 날짜별로 빠르게 매진된 순서를 실시간으로 추적하고 랭킹을 제공하는 기능을 구현함으로써 인기 있는 콘서트 날짜를 파악하고 사용자에게 제공

### 1.2 기술 스택 및 선택 이유

**Redis Sorted Set**을 사용
...
기존 JPA 기반 대기열 시스템을 Redis 기반으로 개선하여, 대용량 트래픽 상황에서도 효율적으로 대기열을 관리하고 사용자에게 실시간 대기 순서를 제공

### 2.2 기술 스택 및 선택 이유

**Redis Sorted Set + Hash**를 사용하여 대기열을 구현하였음.
